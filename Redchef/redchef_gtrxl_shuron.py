...
class Metasploit:
  ...
  #      p.sendline("nmap 192.168.56.101")
   ...
   #     x = results['172.17.0.9']['ports']
    ...
    def reset_state2(self, exploit_tree, target_tree, rhost): 
        self.state = [] 
        nmap_result = [] 
   ...
   #     for i in x:
    ...
    def get_available_actions(self, payload_list, com_payload_list):
        payload_num_list = []
        payload_list2 = []
    ...
    def set_options(self, target_info, target, selected_payload, exploit_tree, rhost):
        ...
        for key in key_list:
         ...
         #   if len([s for s in self.path_collection if s in key.lower()]) != 0:
            ...
            if options[key]['required'] is True:
              ...
              #  print(option[key])
             #   print(target_info['target_path'])

            # Set target path/uri/dir etc.
          #  for s in self.path_collection:
               # if s in key.lower():
             #      print(s)
         #   if len([s for s in self.path_collection if s in key.lower()]) != 0:
           # if len(option[key]) == '':
     ...
     #   else:
        ...
        try:
            if len(options['RPORT']['user_specify']) != 0:
               option['RPORT'] = options['RPORT']['user_specify']
    ...
    # Execute exploit.
    def execute_exploit(self, s, action, thread_name, thread_type, target_list, target_info, step, exploit_tree, rhost):
        # Set target.
        frame=0
        target = ''
        ...
        # Set options.
        option = self.set_options(target_info, target, selected_payload, exploit_tree, rhost)
        print(option)
        # Execute exploit.
        reward = 0
        message = ''
...
MAX_TRAIN_NUM = 300 # Learning number of each thread.
...
class StableTransformerEncoderLayerXL(torch.nn.Module):
    def __init__(
        ...
        self.gating = gating
      #  self.gate1 = GRUGatingUnit(d_input, 2.)
     #   self.gate2 = GRUGatingUnit(d_input, 2.)
        self.gate1 = GRUGate(d_input)
        self.gate2 = GRUGate(d_input)
       # self.gate1 = GatingMechanism(d_input)
     #   self.gate1 = nn.GRUCell(d_input, d_input)#batch_first=True)
      #  self.gate2 = GatingMechanism(d_input)
     #   self.gate2 = nn.GRUCell(d_input, d_input)#batch_first=True)
    ...
    def forward(self, input_, pos_embs, u, v, mask=None, mems=None):
      #  h = (input_).sum(dim=1).unsqueeze(dim=0)
        src2 = self.norm1(input_)
        src2 = self.mha(src2, pos_embs, mems, u, v, mask=mask)
     #   src, h = self.gate1(src2, h) if self.gating else input_ + src2
        o1 = self.gate1(input_, src2) if self.gating else input_ + src2
        src2 = self.ff(self.norm2(o1))
        o2 = self.gate2(o1, src2) if self.gating else o1 + src2
      #  src, h = self.gate2(src2, h) if self.gating else src + src2
        return o2
...
class CommandScorer(nn.Module):
    def __init__(self, input_size, hidden_size):
      ...
      #  self.encoder_gru  = nn.GRU(hidden_size, hidden_size, bidirectional=True)
        self.encoder_model = StableTransformerXL(d_input=512, n_layers=12, 
            n_heads=8, d_head_inner=64, d_ff_inner=512)
        
        self.reporter = MemReporter(self.encoder_model)
    #    self.encoder_model = ding.torch_utils.network.gtrxl.GTrXL(
       #     input_dim = 256, 
     #       head_dim = 64, 
       #     embedding_dim = 256, 
        #    head_num = 8, 
     ...
     #       memory_len = 512, 
      ...
      #      activation = nn.ReLU(), 
       ...
       #     gru_bias = 2.0, 
        ...
        self.memory = None
        self.cmd_encoder_gru  = nn.GRU(hidden_size, self.hidden_size2)
        self.state_gru    = nn.GRU(hidden_size, self.hidden_size2)#dropout=0.3)
       # self.hidden_size2 = self.hidden_size2
        self.linear_size = self.hidden_size2
        self.state_hidden = torch.zeros(1, 1, self.hidden_size2, device=device)
   ...
   #     self.model2 = AutoModelForMaskedLM.from_pretrained(self.download_path, config=self.model_config)
    ...
    def forward(self, obs, commands, **kwargs):
      ...
      #  print(obs)
       ...
       # print(embedded)
        ...
        trans_state = self.encoder_model(embedded, self.memory)
        
        encoder_hidden, self.memory = trans_state['logits'], trans_state['memory']
    ...
    #    encoder_hidden2d = TSNE(n_components=2).fit_transform(encoder_hidden2)
     ...
     #   print(encoder_hidden.shape)
      ...
      #  state_hidden2 = torch.reshape(state_hidden,(1,1,self.hidden_size*2))
    ...
    def reset_hidden(self, batch_size):
        self.state_hidden = torch.zeros(1, batch_size, self.hidden_size2, device=device)
 ...
 #   device = 'cpu'
  ...
  #  def infos_to_request(self) -> EnvInfos:
    ...
    def _process(self, texts):
      ...
      #  print(texts)
        max_len2 = max(len(l) for l in texts)
     #   print(max_len2)
        max_len = 600 #max(len(l) for l in texts)
    ...
    def act(self, obs: str, score: int, done: bool, actions: Mapping[str, Any]) -> Optional[str]:
      ...
      #  print(score)
       # f = open('rewardredchef_gtrxl_real_multi_sgd2.txt', 'a', encoding='UTF-8') 
      #  f.write(str(score)+'\n')
     #   f.close()
...
class Environment:
    ...
    def __init__(self, name, thread_type, rhost):
     ...
     #   self.rhost_list = ['192.168.56.106', "192.168.56.125", '192.168.56.101', '192.168.56.105', '192.168.56.104', '192.168.56.102',]
        self.rhost_list = ['192.168.56.106', "192.168.56.125", '192.168.56.103', '192.168.56.101', '192.168.56.105', '192.168.56.104', '192.168.56.102',  '192.168.56.108', '192.168.56.109', '192.168.56.110']
      #  self.rhost_list = ['192.168.56.121']
        self.rhost_number = 0
        self.exploit_tree = []
        self.com_payload_list = []
        self.target_tree = []
        self.com_port_list = []
        self.step_count2 = 0
    ...
    def check_port_value(self, port=None, service=None):
        if port is not None:
      ...
      #  self.agent.brain.pull_parameter_server()  # Copy ParameterSever weight to LocalBrain
        ...
        #    skip_flag, s, payload_list, target_list, target_info = self.env.reset_state(exploit_tree, target_tree)
         ...
         #   print(self.episode_count)
           # print(self.rhost_number)
          #  if self.rhost_number == 3:
                 #  self.rhost_number = 0
            if self.flag == True:
                self.step_count = 0
                self.flag = False
                ...
                if self.rhost_number == 10:
                   self.rhost_number = 0
                print('RHOST' + str(self.rhost_number))
                rhost = self.rhost_list[self.rhost_number]
                self.rhost_number += 1
                nmap_result = 'nmap_result_' + rhost + '.xml'
                nmap_command = env.nmap_command + ' ' + nmap_result + ' ' + rhost + '\n'
                env.execute_nmap(rhost, nmap_command, env.nmap_timeout)
                com_port_list, proto_list, info_list = env.get_port_list(nmap_result, rhost)
                com_exploit_list = env.get_exploit_list()
                com_payload_list = env.get_payload_list()
                com_payload_list.append('no payload')
                self.com_payload_list = com_payload_list

    # Create exploit tree.
                self.exploit_tree = env.get_exploit_tree()
                port = None
                service = None
    # Create target host information.
                com_indicate_flag = self.check_port_value(port, service)
                if com_indicate_flag:
                             self.target_tree, self.com_port_list = env.get_target_info_indicate(rhost, proto_list, info_list, port, service)
                else:
                             self.target_tree = env.get_target_info(rhost, proto_list, info_list)
            ...
            #    self.rhost_number += 1
                 ...
                 rhost = self.rhost_list[self.rhost_number - 1]
                # rhost = self.rhost_list[0]
                # self.rhost_number += 1
               #  print('RHOST'+str(self.rhost_number))
           # print(target_tree)
            skip_flag, s, a = self.env.reset_state2(self.exploit_tree, self.target_tree, rhost)
          #  print(s2)  
            ...
            if skip_flag is False:
                 ...
                 #   a = self.agent.act(s, r, done, available_actions_list)
                   # print(available_actions)
                    ...
                    if count == 3:
                       ...
                       count += 1 
                     #  print(a)
                       available_actions, a = self.env.get_available_actions(a,self.com_payload_list)
                      # a = a.remove('windows/x64/meterpreter/bind_named_pipe')
                     #  print(a)
                       for i in a:
                           if 'bind_named_pipe' in i:
                   ...
                   #    print('reward;'+ str(r))                                     
                       ...
                       #  print(s)
                        # print(target_tree)
                         a, label_list, encoder_list = self.agent.act(s, r, done, a)
                         s, a, r, done, target_info = self.env.step(s, a, self.exploit_tree, self.target_tree)
                         if count == 2:
                            target_list = a
                      #   print(a_list)
                ...
                #    f = open('rewardredchef_gtrxl_real_multi_512_head_8_lay_12_GRUGataEX_emb_10server5.txt', 'a', encoding='UTF-8') 
                #    f.write(str(r)+'\n')
                 #   f.close()
                 ...
                 #   self.agent.advantage_push_local_brain(s, a, r, s_)
                  ...
                  #  if done or (step % Tmax == 0):
                    ...
                    if done:
                      ...
                      #  print(a_2)
                        self.agent.act(s, r, done, a)#self.act_list)
             ...
             #   self.util.print_message(OK, 'Thread: {}, Trial num: {}, '
                ...
                # End of learning.
                 ...
                 #   output1 = output1.to('cpu').detach().numpy().copy()
                       ...
                       plt.axis('off')
                       plt.colorbar()
                       plt.savefig("redchefgtrxlmultiTSNE_shuron.png")
                       plt.show()
...
class Worker_thread:
   ...
   # def run(self, exploit_tree, target_tree, saver=None, train_path=None):
    def run(self, saver=None, train_path=None):
